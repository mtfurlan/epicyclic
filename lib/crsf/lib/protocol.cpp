...
static const int Kl = 100;    // linearity constant;
static const float Kr = .026; // range constant;
// TODO verify this is the sign function they meant
#define sign(x) (x > 0 ? 1 : -1)
int8_t get_vertical_speed_packed(int16_t vertical_speed_cm_s)
{
    return (int8_t)(log(abs(vertical_speed_cm_s) / Kl + 1) / Kr) * sign(vertical_speed_cm_s);
}

int16_t get_vertical_speed_cm_s(int8_t vertical_speed_packed)
{
    return (exp(abs(vertical_speed_packed * Kr)) - 1) * Kl * sign(vertical_speed_packed);
}
