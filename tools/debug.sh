...
if [[ "$multicore" == true ]]; then
    ...
    gdb-multiarch --quiet -ex "target extended-remote | openocd -c 'gdb_port pipe' -f interface/cmsis-dap.cfg -c'transport select swd' -c 'adapter speed 5000' -f target/rp2040.cfg -c 'rp2040.core1 configure -gdb-port 3334'"  -x tools/gdbConfig "$binary"
else
    ...
    gdb-multiarch --quiet -ex "target extended-remote | openocd -c 'gdb_port pipe' -f interface/cmsis-dap.cfg -c'transport select swd' -c 'adapter speed 5000' -c 'set USE_CORE 0' -f target/rp2040.cfg"  -x tools/gdbConfig "$binary"
fi
